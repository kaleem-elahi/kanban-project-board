...
export default function lists(state = initialState, action) {
	switch (action.type) {
		...
		case 'FETCH_LANES_FULFILLED':
			return state.withMutations((ctx) => {
				ctx.set('isFetching', false)
					.set('fetchLanes', action.lanes);
				console.log(action, "GET_LIST");

			});
		case MOVE_CARD:
			{
				const newLists = [...state.fetchLanes];
				console.log(newLists);
				console.log("MOVE_CARD::newLists");
